...
import { getRecordKey, isStudentExcluded, isStudentExcludedOnDate } from '../utils/attendanceHelpers';
import { 
  Users, 
  CheckCircle2, 
  AlertTriangle, 
...
export const AnalyticsView: React.FC<AnalyticsViewProps> = ({
  ...
  const gradeStats = useMemo(() => {
    ...
    return grades.map(grade => {
      ...
      const rate = totalEligibleSlots > 0 ? Math.min(100, Math.round((effectivePresent / totalEligibleSlots) * 100)) : 0;
      ...
      return {
        ...
        rate,
        totalEligibleSlots,
        totalAttended: Math.min(totalAttended, totalEligibleSlots),
        presentCount,
        lateCount,
        absentCount
      };
    ...
    <div className="space-y-6">
          ...
          <div key={stat.grade} className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <div className="flex items-baseline justify-between mb-2">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">
                {stat.totalAttended} / {stat.totalEligibleSlots} 누적 출석
              </span>
            </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          ...
          <span className="text-xs text-slate-500">총 {studentRankings.length}명 대상</span>
        </div>
